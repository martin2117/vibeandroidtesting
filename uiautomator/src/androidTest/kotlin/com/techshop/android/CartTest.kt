...
class CartTest : BaseUiAutomatorTest() {
    ...
    fun testTcCart005RemoveIndividualItemFromCart() {
        ...
        val isRemoved = device.wait(Until.gone(byRes("qty-p1")), DEFAULT_TIMEOUT)
        val qtyP1 = findAndWait(byRes("qty-p1"), 500L)
    ...
    fun testTcCart007MinimumOrderValueEnforcement() {
        ...
        val emptyCart = findAndWait(byRes("cart-empty"), 2000L)
        val minOrderError = findAndWait(byRes("min-order-error"), 2000L)
        org.junit.Assert.assertTrue(
            "Expected cart to enforce minimum order value or indicate empty state",
            emptyCart != null || minOrderError != null
        )
    }
